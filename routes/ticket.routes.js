...
router.post('/getlist_by_ticket_no', function (req, res) {
  ...
  if (req.body.ticket_no) {
    matchQuery.ticket_no = req.body.ticket_no;
  }
 
  if (req.body.phaseid) {
    matchQuery.phase = req.body.phaseid;
  }
  
        tickethistoryModel.find(matchQuery, function (err, StateList) {
        if(StateList && StateList.length > 0){
          ...
          res.json({Status:"Error",Message:"Empty List", Data : [] ,Code:500});
        }
       
        }).populate("user_id");
});

router.post('/johnson_getlist_by_station_new', async function (req, res) {
  var matchQuery = {};
  if (req.body.station_id) {
    matchQuery.station_id = req.body.station_id;
  }
  if (req.body.job_id) {
    matchQuery.job_id = req.body.job_id;
  }
  if (req.body.status) {
    matchQuery.status = req.body.status;
  }
  if (req.body.type) {
    matchQuery.type = req.body.type;
  }
 ...
 await ticketModel.find(matchQuery, function (err, StateList) {
...
router.post('/create1', async function (req, res) {
  try {
    await ticketModel.create({
      phase: req.body && req.body.phase_id ? req.body.phase_id : "6437c5858ae7c5e08929053c",
...
router.post('/getlist_by_type', async function (req, res) {
  console.log(req.body);
  var phaseID = req.body.phase_id ? req.body.phase_id:null;
  var matchQuery={
    type: req.body.type, 
    break_down_reported_by: req.body.break_down_reported_by, 
  ...
  var temp_data = await ticketModel.find(matchQuery).populate('station_detail job_detail');
...
router.post('/cmrl_dashboard_count', async function (req, res) {
    ...
    var open_count = await ticketModel.countDocuments({ status: "Open",type:type });
    var inprogress_count = await ticketModel.countDocuments({ status: "Inprogress",type:type });
    var pending_count = await ticketModel.countDocuments({ status: "Pending",type:type });
    var completed_count = await ticketModel.countDocuments({ status: "Completed",type:type });
    var close_count = await ticketModel.countDocuments({ status: "Close",type:type });
...
router.post('/cmrl_dashboard_count1', async function (req, res) {

  var phaseList = await phaseModel.find().sort({"createdAt": 1});
  var type= req.body.type ? req.body.type:"1";
  var tempArr = [];
  phaseList && phaseList.forEach(async element => {
    var phaseid = element._id;
    var open_count = await ticketModel.countDocuments({ status: "Open", phase: phaseid,type:type });
    var inprogress_count = await ticketModel.countDocuments({ status: "Inprogress", phase: phaseid,type:type });
    var pending_count = await ticketModel.countDocuments({ status: "Pending", phase: phaseid,type:type });
    var completed_count = await ticketModel.countDocuments({ status: "Completed", phase: phaseid,type:type });
    var close_count = await ticketModel.countDocuments({ status: "Close", phase: phaseid,type:type });
  ...
  setTimeout(() => {
    ...
    const sortedData = tempArr && tempArr.sort((a, b) => {
      if (a._id < b._id) {
        return -1;
      }
      if (a._id > b._id) {
    ...
    res.json({ Status: "Success", Message: "Ticket List", Data: sortedData, Code: 200 });
  }, 1000);
});
...
router.post('/johnson_dashboard_count1', async function (req, res) {
...
//       openCount: {
//         $sum: {
//           $cond: [{ $eq: ["$tickets.status", "open"] }, 1, 0]
//         }
//       },
//       inProgressCount: {
//         $sum: {
//           $cond: [{ $eq: ["$tickets.status", "inprogress"] }, 1, 0]
//         }
//       }
//     }
//   }

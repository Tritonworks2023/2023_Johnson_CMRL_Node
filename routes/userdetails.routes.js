...
router.post('/authenticate', async function(req, res) {
  ...
  var password = req.body.password;
   console.log("user_email ",user_email, "password", password)
  const userFindData = await userdetailsModel.find({user_email : user_email});
  const userData = await userdetailsModel.find({user_email : user_email, password: password, user_type: 1});
  console.log("userFindData ",userFindData, "userData", userData)
  if(userFindData){
      if(userData){
          const token = await jwt.sign({ sub: userData.user_email }, config.secret, { expiresIn: '7d' });
